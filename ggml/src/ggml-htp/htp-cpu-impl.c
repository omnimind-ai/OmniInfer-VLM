...
static const void * g_orig_wdata = NULL;
static ggml_fp16_t * g_w_ref_f16 = NULL;

static void transfer_layout_w16_a32(struct ggml_tensor * tensor, struct ggml_compute_params * params, void (*ggml_barrier_htp)(struct ggml_threadpool * tp)) {
    ...
    if (weight->type == GGML_TYPE_F16 && act->type == GGML_TYPE_F32) {
        if (params->ith == 0) {
            ...
            g_w_ref_f16 = (ggml_fp16_t *) malloc(sizeof(ggml_fp16_t) * (size_t)(k * n));
            GGML_ASSERT(g_w_ref_f16);
            if ((k % 32 == 0) && (n % 32 == 0)) {
                htp_w16_tile32_to_ggml_f16(w_perm, g_w_ref_f16, (size_t)k, (size_t)n);
            } else {
                for (int ii = 0; ii < k; ++ii) {
                    for (int jj = 0; jj < n; ++jj) {
                        int idx = htp_w16_tile32_index(ii, jj, (int)k, (int)n);
                        g_w_ref_f16[jj * (int)k + ii] = w_perm[idx];  // 列主序
                    }
                }
            }
            g_orig_wdata = weight->data;
            ((struct ggml_tensor *)weight)->data = (void *) g_w_ref_f16;
        }
        ggml_barrier_htp(params->threadpool);
    }
}

static void recover_layout_w16_a32(struct ggml_tensor * tensor, struct ggml_compute_params * params, void (*ggml_barrier_htp)(struct ggml_threadpool * tp)) {
    struct ggml_tensor * weight = tensor->src[0];
    struct ggml_tensor * act = tensor->src[1];
    ...
    if (weight->type == GGML_TYPE_F16 && act->type == GGML_TYPE_F32) {
        ...
        if (params->ith == 0) {
            if (g_orig_wdata) {
                // 恢复原始的 data 指针
                ((struct ggml_tensor *)weight)->data = (void *) g_orig_wdata;
                g_orig_wdata = NULL;
            }
            if (g_w_ref_f16) {
                free(g_w_ref_f16);
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_MUL_MAT:
            {
                bool use_npu = htp_ops_support_op(tensor);

                if (params->ith == 0) {
                    // htp_debug_print_tensor_head_info(tensor);
                    // htp_debug_print_tensor_col0_head("[HTP DEBUG] act", act, k, 8); // print activation
                    // htp_debug_print_tensor_col0_head("[HTP DEBUG] weight", weight, k, 8); // print weight
                }

                if (use_npu) {
                    if(params->ith == 0)
                        fprintf(stderr, "✅use NPU\n");
                    htp_ops_compute_op(params, tensor);
                    // if(params->ith == 0)
                    //     htp_debug_print_xpu_result(tensor, use_npu);

                    // transfer_layout_w16_a32(tensor, params, ggml_barrier_htp);
                    
                    // ggml_compute_forward_mul_mat(params, tensor);
                    
                    // recover_layout_w16_a32(tensor, params, ggml_barrier_htp);
                    // if(params->ith == 0) 
                    //     htp_debug_print_xpu_result(tensor, 0);
                } else {
                    // if(params->ith == 0)
                ...
                // if (params->ith == 0) {
                //     // print first 8 elements of result
                //     htp_debug_print_xpu_result(tensor, use_npu);

                //     // print first 8 element of cpu reference
                //     struct ggml_tensor * weight = tensor->src[0];
                //     struct ggml_tensor * act    = tensor->src[1];
                //     const int64_t k = weight->ne[0];
                //     const int64_t n = weight->ne[1];
                //     htp_debug_print_cpu_ref_col0(weight, act, k, n, 8);
                    
                //     // compute MSE of XPU result vs CPU ref
                //     // check_mul_mat_mse(tensor, weight, act, tensor->name);
                // }
            } break;

...
void llama_kv_cache::apply_ubatch(const slot_info & sinfo, const llama_ubatch & ubatch) {
    ...
    ubatch.token_kv_pos.clear();
    ubatch.token_kv_pos.resize(ubatch.n_tokens, -1);
    ...
    for (uint32_t s = 0; s < sinfo.n_stream(); ++s) {
        for (uint32_t ii = 0; ii < sinfo.size(); ++ii) {
            ...
            ubatch.token_kv_pos[i] = int32_t(idx);
...
void llama_kv_cache::set_input_kq_mask(ggml_tensor * dst, const llama_ubatch * ubatch, bool causal_attn) const {
    ...
    std::vector<int32_t> kv_to_batch_pos(n_kv, -1);
    for (int i = 0; i < (int) ubatch->token_kv_pos.size(); ++i) {
        if (ubatch->token_kv_pos[i] >= 0) {
            kv_to_batch_pos[ubatch->token_kv_pos[i]] = i;
        }
    }
        ...
        for (uint32_t s = 0; s < n_stream; ++s) {
            for (uint32_t ii = 0; ii < n_tps; ++ii) {
                ...
                for (uint32_t j = 0; j < n_kv; ++j) {
                    ...
                    //     continue;
                    // }
                    if (causal_attn && kv_to_batch_pos[j] >= 0 && kv_to_batch_pos[j] > (int32_t)i) {
                        continue;
                    }
